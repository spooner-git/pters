class Shop_search {
    constructor (install_target, external_data){
        this.target = {install: install_target, toolbox:'section_shop_search_toolbox', content:'section_shop_search_content'};

        this.page_name = 'shop_search';
        this.data = {
            searched_data : [],
            search_id:null,
    ...
    render(){
        let top_left = `<span class="icon_left" onclick="layer_popup.close_layer_popup();shop_search_popup.clear();">${CImg.arrow_left()}</span>`;
        ...
        document.querySelector('.popup_shop_search .wrapper_top').style.border = 0;
        PopupBase.top_menu_effect(this.target.install);
    }

    render_loading_image(){
        document.querySelector("#section_shop_search_content").innerHTML =
    ...
    dom_row_toolbox(){
        ...
        let html = `<div class="shop_search_upper_box">
    ...
    dom_row_search_button(){
        ...
        let onclick = ()=>{
            ...
            Shop_search_func.search(data, (data)=>{
    ...
    dom_row_reg_button(){
        ...
        let onclick = ()=>{
            ...
            let external_data = {"shop_id":this.data.selected_member_id, "shop_name":this.data.selected_member_name};
            // layer_popup.open_layer_popup(POPUP_BASIC, POPUP_ADDRESS_SHOP_AUTH, 100, POPUP_FROM_RIGHT, null, ()=>{
            //     shop_auth_popup = new Shop_auth('.popup_shop_auth', external_data);
            // });
    ...
    dom_row_search_input(){
        let id = "shop_search_subject_input";
        ...
        let pattern = "[가-힣ㄱ-ㅎㅏ-ㅣa-zA-Z0-9\-_+.,@一-龠々ぁ-んーァ-ヾ\u318D\u119E\u11A2\u2022\u2025a\u00B7\uFE55]{1,100}";
    ...
    dom_row_member_list(){
        ...
        for(let i=0; i<length; i++){
            ...
            let html = `<article style="display: flex;height: 40px;line-height: 40px;padding: 10px 0;" id="shop_searched_${data.member_id}">
            ...
            $(document).off('click', `#shop_searched_${data.member_id}`).on('click', `#shop_searched_${data.member_id}`, ()=>{
                this.step = 2;
