class Trainer_schedule_history{
    constructor(install_target, trainer_id, callback){
    ...
    render(data){
        let top_left = `<span class="icon_left" onclick="layer_popup.close_layer_popup();trainer_schedule_history.clear();">${CImg.arrow_left()}</span>`;
    ...
    dom_list_content_wrap(content){
        ...
        let manual_event_install = ()=>{
            CComp.scroll_container_event_install(wrap_id, ()=>{
                ...
                this.this_page++;
                let send_data = {"trainer_id": this.trainer_id, "sort_val": this.sort_val, "page": this.this_page};
                Trainer_func.read_schedule_list(send_data, (data)=>{
    ...
    dom_list_by_time(received_data){
        let length = 0;
        if(received_data.trainer_schedule != undefined){
            length = received_data.trainer_schedule.length;
        }
        let html_to_join = [];
        for(let i=0; i<length; i++){
        // for(let i=0; i<length; i++){
            let data = received_data.trainer_schedule[i];
    ...
    dom_list_by_monthly(received_data){
        ...
        let trainer_monthly_list = [];
        for(let month_num in received_data){

            trainer_monthly_list.push(received_data[month_num]);
        }
        trainer_monthly_list.sort(function(a, b){
            let return_val = 0;
        ...
        if(this.expand_monthly == null){
            this.expand_monthly = {};
            for(let i=trainer_monthly_list.length-1; i>=0; i--){
                this.expand_monthly[i+1] = SHOW;
            }
        }
        for(let i=trainer_monthly_list.length-1; i>=0; i--){
            let length = trainer_monthly_list[i].schedule_data.length;
            let html_sub_assembly_to_join = [];
            let expand_button;
            let expand_status;
            let expand_style;
            let month_num_split = trainer_monthly_list[i].month_num.split('-');
            let month_num = month_num_split[0]+'년 ' + month_num_split[1] + '월 ';

            for(let j=length-1; j>=0; j--){
                let data = trainer_monthly_list[i].schedule_data[j];
                let schedule_id = data.schedule_id;
            ...
            let html_sub_assembly = ` <div id="trainer_schedule_history_month_${i+1}">
                                        <div id="trainer_schedule_history_monthly_${i+1}" style="padding:15px 10px;background-color:var(--bg-light);font-size:14px;font-weight:bold;">
                                            <div>${month_num} <span style="float:right;">${expand_button}</span></div>
                                            
                                        </div>
                                        <div id="trainer_schedule_history_month_${i+1}_list" data-expand="${expand_status}" style="display:${expand_style};">
        ...
        if(trainer_monthly_list.length == 0){
            html_to_join.push(`<div style="font-size:14px;padding:16px;">내역이 없습니다.</div>`);
        }
