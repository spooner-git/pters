class ProgramBoard_add{
    constructor(install_target, callback){
        this.target = {install: install_target, toolbox:'section_program_board_add_toolbox', content:'section_program_board_add_content'};
        this.data_sending_now = false;
        this.callback = callback;
        this.form_id = 'id_program_board_add_form';
    ...
    render(){
        let top_left = `<span class="icon_left" onclick="layer_popup.close_layer_popup();program_board_add_popup.clear();">${CImg.x()}</span>`;
        let top_center = `<span class="icon_center"><span id="ticket_name_in_popup">&nbsp;</span></span>`;
        let top_right = `<span class="icon_right" onclick="program_board_add_popup.send_data();"><span style="color:var(--font-highlight);font-weight: 500;">등록</span></span>`;
        ...
        document.querySelector('.popup_program_board_add .wrapper_top').style.border = 0;
    ...
    dom_assembly_content(){
        ...
        let name = this.dom_row_program_board_name_input();
        let note = this.dom_row_program_board_note_input();
    ...
    dom_row_toolbox(){
        let title = "새로운 게시판";
        let html = `
        <div class="program_board_add_upper_box">
    ...
    dom_row_program_board_name_input(){
        let id = 'input_program_board_name';
    ...
    dom_row_program_board_note_input(){
    ...
    send_data(){
        let auth_inspect = pass_inspector.program_board_create();
        ...
        ProgramBoard_func.create(data, (received)=>{
            this.data_sending_now = false;
            try{
                current_page.init();
            }catch(e){}
