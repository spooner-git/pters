class ProgramNotice_view{
    constructor(install_target, program_notice_id, instance){
        this.target = {install: install_target, toolbox:'section_program_notice_view_toolbox', content:'section_program_notice_view_content'};
        this.instance = instance;
        this.program_notice_id = program_notice_id;
        this.form_id = 'id_program_notice_view_form';
    ...
    set_initial_data (){
        ProgramNotice_func.read({"program_notice_id": this.program_notice_id}, (data)=>{
            this.data.name = data.program_notice_name;
            this.data.note = data.program_notice_note;
            this.data.board_type_cd = data.program_notice_type_cd;
            this.data.board_type_cd_name = PROGRAM_BOARD_TYPE[this.data.program_notice_type_cd];
            this.data.use = data.program_notice_use;
            this.render();
    ...
    render(){
        let top_left = `<span class="icon_left" onclick="program_notice_view_popup.upper_left_menu();">${CImg.arrow_left()}</span>`;
        let top_center = `<span class="icon_center"><span>&nbsp;</span></span>`;
        let top_right = `<span class="icon_right" onclick="program_notice_view_popup.upper_right_menu();">${CImg.more()}</span>`;
        ...
        document.querySelector('.popup_program_notice_view .wrapper_top').style.border = 0;
    ...
    dom_assembly_content(){
        ...
        let note = this.dom_row_program_notice_note_view();
    ...
    dom_row_toolbox(){
        ...
        let html = `
        <div class="program_notice_view_upper_box">
    ...
    dom_row_program_notice_note_view(){
    ...
    send_data(){
        ...
        let data = {
            "program_notice_id":this.program_notice_id,
    ...
    upper_right_menu(){
        let user_option = {
            activate:{text:"활성화", callback:()=>{
                    ...
                    show_user_confirm(message, ()=>{
                        ...
                        ProgramNotice_func.update({"program_notice_id":this.program_notice_id, "use":ON}, ()=>{
                            try{
                                current_page.init();
                            }catch(e){}
                            try{
            ...
            deactivate:{text:"비활성화", callback:()=>{
                    ...
                    show_user_confirm(message, ()=>{
                        ProgramNotice_func.update({"program_notice_id":this.program_notice_id, "use":OFF}, ()=>{
                            try{
                                current_page.init();
                            }catch(e){}
                            try{
            ...
            delete:{text:"삭제", callback:()=>{
                    ...
                    show_user_confirm(message, ()=>{
                        ProgramNotice_func.delete({"program_notice_id":this.program_notice_id}, ()=>{
                            try{
                                current_page.init();
                            }catch(e){}
                            try{
        ...
        if(this.data.program_notice_state == STATE_IN_PROGRESS){
            delete user_option.activate;
            delete user_option.delete;
        }else if(this.data.program_notice_state == STATE_END_PROGRESS){
    ...
    func_update_fixed_member(){
        ...
        let member_ids = Object.keys(members); //data_original과 data의 program_notice_id들을 중복을 제거하고 합친 결과
    ...
    check_before_send(){
        ...
        if(error_info != ''){
            ...
            if(this.data.capacity <= 1 && this.data.program_notice_type_cd != program_notice_TYPE_ONE_TO_ONE){
