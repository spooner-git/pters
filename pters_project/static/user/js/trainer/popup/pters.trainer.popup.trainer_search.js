class Trainer_search {
    constructor (install_target, external_data){
        this.target = {install: install_target, toolbox:'section_trainer_search_toolbox', content:'section_trainer_search_content'};

        this.page_name = 'trainer_search';
        this.data = {
            searched_data : [],
            search_id:null,
    ...
    render(){
        let top_left = `<span class="icon_left" onclick="layer_popup.close_layer_popup();trainer_search_popup.clear();">${CImg.arrow_left()}</span>`;
        ...
        document.querySelector('.popup_trainer_search .wrapper_top').style.border = 0;
        PopupBase.top_menu_effect(this.target.install);
    }

    render_loading_image(){
        document.querySelector("#section_trainer_search_content").innerHTML =
    ...
    dom_row_toolbox(){
        ...
        let html = `<div class="trainer_search_upper_box">
    ...
    dom_row_search_button(){
        ...
        let onclick = ()=>{
            ...
            Trainer_search_func.search(data, (data)=>{
    ...
    dom_row_reg_button(){
        ...
        let onclick = ()=>{
            ...
            layer_popup.open_layer_popup(POPUP_BASIC, POPUP_ADDRESS_TRAINER_ADD, 100, popup_style, null, ()=>{
                trainer_add_popup = new Trainer_add('.popup_trainer_add', external_data, 'trainer_add_popup');});
        };
        let html = CComponent.button (id, title, style, onclick);
        return html;
    }
    ...
    dom_row_search_input(){
        let id = "trainer_search_subject_input";
    ...
    dom_row_member_list(){
        ...
        for(let i=0; i<length; i++){
            ...
            let html = `<article style="display: flex;height: 40px;line-height: 40px;padding: 10px 0;" id="trainer_searched_${data.member_id}">
            ...
            $(document).off('click', `#trainer_searched_${data.member_id}`).on('click', `#trainer_searched_${data.member_id}`, ()=>{
                this.step = 2;
