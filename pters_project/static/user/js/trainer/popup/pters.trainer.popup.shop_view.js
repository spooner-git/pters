class Shop_view{
    constructor(install_target, shop_id, instance){
    ...
    set_initial_data (){
        Shop_func.read({"shop_id": this.shop_id}, (data)=>{
            this.data.user_id = data.shop_user_id;
            this.data.name = data.shop_name;
            this.data.phone = data.shop_phone == "None" || data.shop_phone == "" ? null : data.shop_phone;
            this.data.birth = DateRobot.to_split(data.shop_birthday_dt);
            this.data.sex = data.shop_sex == "None" || data.shop_sex == "" ? null : data.shop_sex;
            this.data.connection = data.shop_connection_check;
            this.data.active = data.shop_is_active;
            this.data.email = data.shop_email;
            this.data.profile_img = data.shop_profile_url;
            Shop_auth_func.read({"shop_id":this.shop_id}, (data)=>{
                let my_auth = data[this.shop_id];
                if(my_auth != undefined){
                    ...
                    this.data.auth.trainer.create = my_auth.auth_shop_create != undefined ? my_auth.auth_shop_create : OFF;
                    this.data.auth.trainer.read = my_auth.auth_shop_read != undefined ? my_auth.auth_shop_read : OFF;
                    this.data.auth.trainer.update = my_auth.auth_shop_update != undefined ? my_auth.auth_shop_update : OFF;
                    this.data.auth.trainer.delete = my_auth.auth_shop_delete != undefined ? my_auth.auth_shop_delete : OFF;
                ...
                Shop_func.repeat_list(
                    {"shop_id":this.shop_id}, (data)=>{
                        let repeat_list = data.shop_repeat_schedule_data; //array

                        this.data.repeat = repeat_list;

                        Shop_func.lecture_list(
                            {"shop_id":this.shop_id}, (data)=> {
                                this.data.lecture = data.shop_lecture_data;
                                this.render();
                            });
                    }
                );
    ...
    dom_assembly_content(){
        let user_id = this.dom_row_shop_user_id_input();
                ...
                let user_option = {
                        connect:{text:"연결 해제", callback:()=>{
                            show_user_confirm ({title:`정말 연결을 해제하시겠습니까?`}, ()=>{
                                ...
                                layer_popup.close_layer_popup();
                                let data = {"shop_id":this.shop_id, "shop_auth_cd":AUTH_TYPE_DELETE};
                                Shop_func.connection(data, ()=>{
                                    this.set_initial_data();
                                    current_page.reset();
                                });
                            });
                ...
                let user_option = {
                        connect:{text:"연결 요청", callback:()=>{
                            show_user_confirm ({title:`연결 요청을 보내시겠습니까?`}, ()=>{
                                layer_popup.close_layer_popup();
                                layer_popup.close_layer_popup();
                                let data = {"shop_id":this.shop_id, "shop_auth_cd":AUTH_TYPE_WAIT};
                                Shop_func.connection(data, ()=>{
                                    this.set_initial_data();
                                    current_page.reset();
                                });
    ...
    dom_row_lecture_item(lecture_id, color, lecture_name, lecture_max_num, lecture_minute){
        let onclick = `shop_view_popup.event_view_lecture(${lecture_id})`;
    ...
    dom_sub_assembly_trainer(){
        ...
        let shop_auth_create = this.dom_row_share_menu_auth_toggle("강사", "등록", "ticket", "create");
        let shop_auth_read = this.dom_row_share_menu_auth_toggle("강사", "조회", "ticket", "read");
        let shop_auth_update = this.dom_row_share_menu_auth_toggle("강사", "수정", "ticket", "update");
        let shop_auth_delete = this.dom_row_share_menu_auth_toggle("강사", "삭제", "ticket", "delete");

        let child_assemble = this.data.auth.trainer.read == ON ? shop_auth_read + shop_auth_create +  shop_auth_update + shop_auth_delete : "";
    ...
    event_edit_photo(edit_enable){
        let user_option = {
            ...
            delete:{text:"프로필 사진 삭제", callback:()=>{
                    ...
                    let data = {"shop_id": this.shop_id};
                    let self = this;
                    $.ajax({
                        url: '/trainer/delete_shop_profile_img/',
                        dataType : 'html',
                        data: data,
                        type:'POST',
    ...
    send_data(success_callback){
        ...
        let data_for_auth = {
            ...
            "auth_shop_create":this.data.auth.trainer.create,
            "auth_shop_read":this.data.auth.trainer.read,
            "auth_shop_update":this.data.auth.trainer.update,
            "auth_shop_delete":this.data.auth.trainer.delete,
        ...
        if(this.if_user_changed_any_information==true && this.if_user_changed_auth_any_information==true){
            Shop_func.update(data, ()=>{
                Shop_auth_func.update(data_for_auth, ()=>{
                    this.set_initial_data();
...
class Shop_simple_view{
    constructor(install_target, shop_id, instance){
        this.target = {install: install_target, toolbox:'section_shop_simple_view_toolbox', content:'section_shop_simple_view_content', close_button:'section_shop_simple_view_close_button'};
        this.instance = instance;
        this.shop_id = shop_id;
    ...
    set_initial_data (){
        Shop_func.read({"shop_id": this.shop_id}, (data)=>{
            this.data.user_id = data.shop_user_id;
            this.data.name = data.shop_name;
            this.data.phone = data.shop_phone;
            this.data.birth = data.shop_birthday_dt;
            this.data.sex = data.shop_sex;
            this.data.connection = data.shop_connection_check;
            this.data.active = data.shop_is_active;
            this.data.email = data.shop_email;
            this.render();
    ...
    dom_assembly_content(){
        ...
        let id = this.dom_row_shop_user_id();
        let connection = this.dom_row_shop_connection();
        let phone = this.dom_row_shop_phone_input();
        let birth = this.dom_row_shop_birth_input();
        let sex = this.dom_row_shop_sex_input();
        // let memo = this.dom_row_shop_memo_input();
    ...
    dom_row_shop_connection(){
        let id = 'shop_connection_view';
        let title = "";
        if(this.data.connection == CONNECTED){
    ...
    dom_close_button(){
        ...
        let html = CComponent.button ("close_shop_simple", "닫기", style, ()=>{
            layer_popup.close_layer_popup();
        });
        return html;
    }
