...
function startTimeArraySet(selecteddate, jsondata, Timeunit, filter){ //offAddOkArray 채우기 : 시작시간 리스트 채우기!!!!
    ...
    // console.log("결과s", new_disable_time_array_start_date);
    // console.log("결과e", new_disable_time_array_end_date);
    ...
    for(var i=1; i<=len; i++){
        if($('#groupParticipants div.groupParticipantsRow[data-dbid="'+jsondata.db_id[i-1]+'"]').length == 0){
            ...
            htmlToJoin[i] = '<div class="list_addByList_padding list_addByList" data-lastname="'+jsondata.last_name[i-1]+
                ...
                sexInfo+jsondata.name[i-1]+'<p style="margin:0;">'+'('+jsondata.member_id[i-1]+')</p>'+'</div>'+
                '<div>'+jsondata.avail_count[i-1]+'</div>'+
                '<div style="display:none;">'+jsondata.phone[i-1]+'</div>'+'<div><img src="/static/user/res/floatbtn/btn-plus.png" class="add_listedMember"></div>'+'</div>';
        }
    }
    if(len == 0){
...
function send_plan_delete(option, callbackoption, callback){
    ...
    $('#id_auth_member_num').val(auth_type_cd['auth_member_update'].limit_num);
