// 달력 initialize
let draw_month_calendar_table = (function () {
    // //달력 사용시 커스텀 디자인 적용 가이드
    let default_design_options = {"font_color_sunday":["obj_font_color_pters_dark_red"],
    ...
    let custom_design_options = {"font_color_sunday":"obj_font_color_light_blue",
    ...
    init_month_calendar_basic_size(calendar_height);

    return function draw_month_calendar_table(reference_date, targetHTML) {
...
function init_month_calendar(reference_date, targetHTML){
    ...
    draw_month_calendar_table(reference_date, targetHTML);
    set_touch_move_to_month_calendar(targetHTML);
    // init_month_calendar_basic_size();
}

function init_month_calendar_basic_size(calendar_height){
...
function func_draw_month_calendar_table(reference_date, targetHTML, design_options){
    let lastday_array = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];      //각 달의 일수
    let $targetHTML = $(targetHTML);
    let referencedate_split_array = reference_date.split('-');
    let referencedate_year = referencedate_split_array[0];
    let referencedate_month = referencedate_split_array[1];
    let referencedate_date = referencedate_split_array[2];
    ...
    let current_month_firstdate_day = new Date(`${referencedate_year}`,`${referencedate_month-1}`,`${'1'}`).getDay();

    let prev_next_month = get_prev_next_month(referencedate_year, referencedate_month);
    let prev_page = prev_next_month["prev"];
    let next_page = prev_next_month["next"];

    //달력의 상단의 연월 표기
    let month_calendar_upper_tool = `<div id="pters_month_cal_upper_tool_box">
                                        <div id="go_prev_month" onclick="draw_month_calendar_table('${prev_page}', '${targetHTML}')">
                                        ...
                                        <div style="display:inline-block;vertical-align:middle;">
                                            <div id="pters_month_cal_tool_year_text" class="obj_font_size_12_weight_500">
                                                ${Number(referencedate_year)}년
                                            </div>
                                            <div id="pters_month_cal_tool_month_text" class="obj_font_size_20_weight_bold">
                                                ${Number(referencedate_month)}월
                                            </div>
                                        </div>
                                        <div id="go_next_month" onclick="draw_month_calendar_table('${next_page}', '${targetHTML}')">
                                            <img src="/static/user/res/icon-setting-arrow.png">
                                        </div>
                                    </div>`;
    ...
    for(let i=1; i<=6; i++){
        ...
        for(let j=0; j<7; j++){
            let data_date = `${referencedate_year}-${referencedate_month}-${date_cache}`;
            let font_color = "";
            if(j == 0){
...
function get_prev_next_month(reference_date_year, reference_date_month){
    let lastday_array = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];      //각 달의 일수
    if( (reference_date_year % 4 == 0 && reference_date_year % 100 != 0) || reference_date_year % 400 == 0 ){  //윤년
        lastday_array[1] = 29;
    }else{
        lastday_array[1] = 28;
    }
    ...
    let next_month = new Date(`${reference_date_year}`,`${reference_date_month-1}`,`${lastday_array[reference_date_month-1]}`);

    prev_month.setDate(0);
    next_month.setDate(next_month.getDate()+1);
    let prev_page = `${prev_month.getFullYear()}-${prev_month.getMonth()+1}-${prev_month.getDate()}`;
    let next_page = `${next_month.getFullYear()}-${next_month.getMonth()+1}-${next_month.getDate()}`;
