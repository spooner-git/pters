...
function delete_group_from_list(group_id){
    var bodywidth = window.innerWidth;
    var next_page = '/center/get_group_ing_list'
    if($('#currentGroupList').css('display') == "block"){
        next_page = '/center/get_group_ing_list'
    }else if($('#finishedGroupList').css('display') == "block"){
        next_page = '/center/get_group_end_list'
    }
    $.ajax({
        url:'/center/delete_group_info/',
        type:'POST',
...
function delete_groupmember_from_grouplist(use, callback){
    ...
    $.ajax({
        url:'/center/delete_group_member_info/',
        type:'POST',
        data:JSON.stringify(group_delete_JSON),
        //data:{"member_name":fullname, "member_id":id, "group_id":group_id, "next_page":'/center/get_group_info/'},
        dataType : 'html',
...
function modify_group_status(group_id, option){
    var bodywidth = window.innerWidth;
    if(option == 'complete'){
        var _URL = '/center/finish_group_info/'
    }else if(option == 'resume'){
        var _URL = '/center/progress_group_info/'
    }
...
function groupMemberListSet(group_id, jsondata){
    ...
    if(Options.auth_class == 0){
        var addButton = '<div><img src="/static/user/res/login/icon-lock-grey.png" style="width:20px;margin-top: 10px;margin-bottom:10px;"  title="기능 구매 후 이용가능" onclick="purchase_annai()"></div>'
    }else{
...
function get_group_repeat_info(group_id){
    $.ajax({
        url: '/center/get_group_repeat_schedule_list/',
        type:'GET',
        data: {"group_id": group_id},
        dataType : 'html',
