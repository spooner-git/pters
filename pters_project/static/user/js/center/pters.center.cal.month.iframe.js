...
function classDatesTrainer(jsondata){
    ...
    for(var i=0; i<len; i++){
        ...
        if(yy+mm+dd < oriYear+omm+odd){  // 지난 일정은 회색으로, 앞으로 일정은 핑크색으로 표기
            ...
            $("td[data-date="+dateTarget+"] div._classTime").addClass('balloon_center').html('<img src="/static/user/res/icon-cal-mini.png">'+countResult[i])
            $("td[data-date="+dateTarget+"] div._classDate").addClass('greydateMytime')
        }else{
            $("td[data-date="+dateTarget+"]").attr('schedule-id',jsondata.scheduleIdArray[i])
            $("td[data-date="+dateTarget+"] div._classTime").addClass('blackballoon_center').html('<img src="/static/user/res/icon-cal-mini.png">'+countResult[i])
