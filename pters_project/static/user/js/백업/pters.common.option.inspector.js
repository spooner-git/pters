        ...
        //지점
            ...
            // if(auth_type_cd.auth_program_create.limit_num > 0){
                ...
                if(current_program_num >= limit_num){
                    show_caution_popup(function_lock_message("create", limit_num, "지점 생성", limit_type));
                }else{
            ...
            if(limit_num == 0){
                show_caution_popup(function_lock_message("delete", 1, "지점 삭제", limit_type));
                if(xhr != ""){
                    xhr.abort(); // ajax중지
                    completeSend(); // ajax 로딩 이미지 숨기기
            ...
            if(limit_num == 0){
                show_caution_popup(function_lock_message("delete", 1, "지점명 수정", limit_type));
            }else if(limit_num > 0){
...
function function_lock_message(function_type, number, option_type, user_type){ //auth_type_cd_lock_date
    ...
    var free_message1 = '';
    if(device_info != "web"){
        free_message1 = `<span style="color:#fe4e65;">추가 이용권</span>으로<br><span style="color:#fe4e65;">제한 없이 </span>이용 해보세요!`;
    }else{
        free_message1 = `<span style="color:#fe4e65;">추가 이용권</span>으로<br><span style="color:#fe4e65;">제한 없이 </span>이용 해보세요!
...
function show_free_member_use_guide(view_page){
    ...
    if(view_page == "plan"){
        ...
        if(auth_type_cd.auth_member_create.limit_type == "무료"){
            $popup.show().text(`(${auth_type_cd.auth_member_create.limit_type}이용자) 회원 등록: 최대 ${auth_type_cd.auth_member_create.limit_num}명 등록 가능(진행중)`);
        }
    }else if(view_page == "lecture_manage"){
        if(auth_type_cd.auth_group_create.limit_type == "무료"){
            $popup.show().text(`(${auth_type_cd.auth_group_create.limit_type}이용자) 그룹 등록: 최대 ${auth_type_cd.auth_group_create.limit_num}개 등록 가능(진행중)`);
        }
        ...
        if(auth_type_cd.auth_program_create.limit_type == "무료"){
            $popup.show().text(`(${auth_type_cd.auth_program_create.limit_type}이용자) 지점: 최대 ${auth_type_cd.auth_program_create.limit_num}개 생성 가능`);
        }
